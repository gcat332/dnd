...
**Status**: in progress. Task 1 (dice notation parsing + roll result model, client-only) merged to this branch. Task 2 (`0007_dice_rolls.sql` migration + `roll_dice` RPC + integration test, commit `48f3bf2`) done: `npm run test:db` 20/20, `npm test` 206/206, `npm run build` clean. Next: Task 3 (`api.ts` + `DiceRollerPanel` UI, calls `roll_dice` by exact name/params).
