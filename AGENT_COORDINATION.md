...
**Status**: DONE — all 4 tasks of the plan merged to this branch (`feat/taleforge-dice-roller`). Task 1 (notation parsing + roll result model), Task 2 (`0007_dice_rolls.sql` migration + `roll_dice` RPC), Task 3 (`api.ts` — `rollDice`/`listRecentRolls`), Task 4 (`DiceRollerPanel.tsx` rendered on `CampaignDashboardPage.tsx`, commit `27f717f`). Final state: `npm test` 213/213, `npm run build` clean (zero TS errors), `npm run test:db` 20/20 (dice RPC round-trip still green). Any campaign member can now roll dice by notation from the dashboard; server computes the result (uncheatable), persists to `dice_rolls`, and the panel shows history. Deferred: rolling from the battle map / attaching to a token (needs Codex's battle-map area), folding into a Session Log (#2), advantage/disadvantage/exploding dice, realtime push (#9).
