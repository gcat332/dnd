...
**Battle Map**:
A square-grid tactical play space presented as a 2.5D tabletop through an
adjustable orthographic view, with elevation, dimensional terrain and tokens,
lighting, shadows, and animation while remaining one map coordinate space
rather than a freely navigable 3D world.
...
**Enforcement Preset**:
A named starting configuration (Strict, Balanced, Narrative) that sets Hard/Warn/Advisory defaults across tactical rule categories for a campaign; the DM can override any category afterward.
_Avoid_: Difficulty setting, single global toggle
